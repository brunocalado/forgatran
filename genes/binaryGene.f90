!> Binary gene with one dimension. \n\n
!! Enable it with: USE binary in the main file.
MODULE binaryGene
    USE parameters
    USE sorter
    USE randomga
...
!        CALL scorePop(1, popSize) ! FROM 1 to popSize
    ...
    FUNCTION createGene()
        IMPLICIT NONE !Não permite variaves não declaradas
        LOGICAL (KIND=logicalSize)    :: createGene(totalGeneSize)
        INTEGER (KIND=integerSize)    :: i
        DO i=1, totalGeneSize
            createGene(i) =  getRandomLogical()
        END DO
    END FUNCTION
        ...
        DO i=1, offspringSize
            DO j=1, totalGeneSize
                IF( getRealRandom() <= mutateRate ) THEN
                    popAux(j, i) = .NOT. popAux(j, i)
                    stat_mutationsPerGeneration = stat_mutationsPerGeneration + 1
                END IF
            END DO
        END DO
    END SUBROUTINE
    ...
    SUBROUTINE mutatePlus
        ...
        DO i=1, offspringSize
            DO j=1, mutationsPerGene
                ...
                popAux(mutationPos, i) = .NOT. popAux(mutationPos, i)
                stat_mutationsPerGeneration = stat_mutationsPerGeneration + 1
            END DO
        END DO
...
!------------------------------------------------------------------------------
    SUBROUTINE viewPop(n)
        ...
        20 FORMAT( <geneSize*yGeneSize> L1 )
    END SUBROUTINE
    ...
    SUBROUTINE loadPop(file)
        IMPLICIT NONE
        CHARACTER (LEN=*, KIND=characterSize), INTENT(IN)    :: file
        LOGICAL (KIND=logicalSize)                            :: gene(geneSize*yGeneSize)
        INTEGER (KIND=integerSize)                             :: i, iostatus
